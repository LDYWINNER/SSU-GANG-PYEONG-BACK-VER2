...
export class BoardUpdate1718602291484 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "board" ADD "title" character varying NOT NULL`,
    );
    await queryRunner.query(`ALTER TABLE "board" ADD "views" integer NOT NULL`);
    await queryRunner.query(
      `ALTER TABLE "board" ADD "category" character varying NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "board" ADD "anonymity" boolean NOT NULL`,
    );
  }
